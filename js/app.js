(() => {
    ...
    if (menuList) {
        menuList.addEventListener("mouseover", (e => {
            if (e.target.classList.contains("menu__link")) {
                menuList.style.setProperty("--underline-width", `${e.target.offsetWidth}px`);
                menuList.style.setProperty("--underline-offset-x", `${e.target.offsetLeft}px`);
            }
        }));
        menuList.addEventListener("mouseleave", (() => menuList.style.setProperty("--underline-width", "0")));
    }
    const menuItems = document.querySelectorAll(".menu__item");
    if (menuItems.length) {
        let delay = .3;
        menuItems.forEach((item => {
            item.style.setProperty("--delay", `${delay += .2}s`);
        }));
    }
    const gradientTextItems = document.querySelectorAll("[data-gradient-animation]");
    if (gradientTextItems.length) gradientTextItems.forEach((gradientTextItem => {
        let gradientTextItemColor = window.getComputedStyle(gradientTextItem).color;
        gradientTextItem.style.setProperty(`--text-color`, `${gradientTextItemColor}`);
    }));
    const creatorsPageCard = document.querySelectorAll(".creators-page__card");
    if (creatorsPageCard.length) {
        let delay = 0;
        creatorsPageCard.forEach((card => {
            card.style.setProperty("--delay", `${delay += .1}s`);
        }));
    }
    const cards = document.querySelectorAll(".discover-page__card-discover");
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        if (date) {
            let arrDate = date.getAttribute("data-date").split(".");
            const deadline = new Date(arrDate[0], arrDate[1], arrDate[2]);
            let timerId = null;
            function countdownTimer() {
                const diff = deadline - new Date;
                if (diff <= 0) clearInterval(timerId);
                const hours = diff > 0 ? Math.floor(diff / 1e3 / 60 / 60) - 1 + Math.floor(diff / 1e3 / 60 / 60) % 24 : 0;
